...
{!! view_render_event('frooxi.shop.checkout.cart.summary.estimate_shipping.before') !!}

<x-shop::accordion
    class="overflow-hidden rounded-[5px] border border-zinc-200 bg-white"
    :is-active="false"
>
    <x-slot:header class="border-b border-zinc-200 px-5 py-4 text-sm font-medium text-black">
        @lang('shop::app.checkout.cart.summary.estimate-shipping.title')
    </x-slot>

    <x-slot:content class="!p-5 pt-0 max-md:!p-5 max-md:pt-0">
...
{!! view_render_event('frooxi.shop.checkout.cart.summary.estimate_shipping.after') !!}
    ...
    <script type="text/x-template" id="v-estimate-tax-shipping-template">
        ...
        >
            <form @change="handleSubmit($event, estimateShipping)">
                ...
                {!! view_render_event('frooxi.shop.checkout.onepage.address.form.country.after') !!}
                ...
                <div
                    class="mt-4 grid rounded-[5px] border border-zinc-200"
                    v-if="methods.length"
                >
                    <template v-for="method in methods">
                        {!! view_render_event('frooxi.shop.checkout.cart.summary.estimate_shipping.shipping_method.before') !!}
                        ...
                        >
                            ...
                            <label 
                                ...
                                :for="rate.method"
                            >
                                <p class="text-xl font-medium text-black max-md:text-lg">
                                    @{{ rate.base_formatted_price }}
                                </p>
                                
                                <p class="mt-2.5 text-xs font-normal text-zinc-500 max-md:mt-0">
                                    <span class="font-medium">@{{ rate.method_title }}</span> - @{{ rate.method_description }}
                                </p>
                            </label>
                        </div>

                        {!! view_render_event('frooxi.shop.checkout.cart.summary.estimate_shipping.shipping_method.after') !!}
                    </template>
                </div>
            </form>                    
        </x-shop::form>
